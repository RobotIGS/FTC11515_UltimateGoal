...
public class OrientationTools {
    ...
    /**
     * Constructor
     * @param robot Object that inherits from HardwareChassis
     * @param opMode Calling op Mode
     */
    public OrientationTools(BaseHardwareMap robot, LinearOpMode opMode) {
        this.robot = robot;
        hwgy = new HardwareChassisGyro(robot.hwMap);
        hwgy.init(robot.hwMap);
        this.opMode = opMode;
    }
    ...
    public void setAllMotors(double SpeedFrontLeft, double SpeedBackLeft, double SpeedBackRight,double SpeedFrontRight){
        robot.motor_front_left.setPower(SpeedFrontLeft);
        robot.motor_front_right.setPower(SpeedFrontRight);
        robot.motor_rear_left.setPower(SpeedBackLeft);
        robot.motor_rear_right.setPower(SpeedBackRight);
    }
    ...
    public void driveSidewardEncoder(OpMode op,double distanceForward, double distanceSideways, double speed,OmniWheel wheel, double startPos, BNO055IMU imu, double smoothness, double smoothnessAdjust) {
        ...
        double[] wheelSpeeds = OmniWheel.calculate( distanceForward / maxDistance, distanceSideways / maxDistance, 0);
